...
class TestTextReportService:
    async def test_export_report_success(
        self, service, mock_upload_file, mock_background_tasks
    ):
        ...
        with (
            ...
            response = await service.export_report(
                file=mock_upload_file, background_tasks=mock_background_tasks
            )
    ...
    async def test_export_report_no_filename(self, service, mock_background_tasks):
        ...
        with pytest.raises(BadRequestException) as exc:
            await service.export_report(
                file=mock_file, background_tasks=mock_background_tasks
            )

        assert "The file was not transferred" in str(exc.value.detail)

    async def test_export_report_internal_error(
        self, service, mock_upload_file, mock_background_tasks
    ):
        """Проверка обработки внутренних ошибок и вызова очистки."""

        with (
            patch("tempfile.mkstemp", return_value=(1, "temp.path")),
            patch("os.close"),
